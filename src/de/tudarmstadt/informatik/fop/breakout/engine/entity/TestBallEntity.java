...
public class TestBallEntity extends Entity{
	...
	public TestBallEntity(String entityID) {
		super(entityID);
			...
			public void update(GameContainer gc, StateBasedGame sb, int delta, Component event) {
				...
				if (!collidedEntity.isPassable()) {
					...
					if (collidedEntity.getID().contains("Border")
						...
						TestBlockEntity block = (TestBlockEntity) collidedEntity;
	...
	public void duplicateBall() {
		if (LevelHandler.ballListHasSpace()) {
			// creating a new ball
			BallEntity ball2 = new BallEntity("ball2");
			// putting the new ball at the position of the already existing ball
			ball2.setPosition(getPosition());
